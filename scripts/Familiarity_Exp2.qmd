...
M3_formula <- bmf(
    Correct ~ (1-iP)*(a1+c1) + iP*(a2+c2) + b,
    TransOld ~ a1 + b,
    TransNew ~ a2 + b,
    OutSame ~ iP*(ra*a1 + rc*c1) + b,
    OutOther ~ ra*a1 + b,
    NPL ~ b,
...
M3_model <- m3(
  resp_cats = c("Correct", "TransOld", "TransNew", "OutSame", "OutOther", "NPL"),
  num_options = c("nCorrect", "nTransOld", "nTransNew", "nOutSame", "nOutOther", "nNPL"),
  choice_rule = "luce",
  version = "custom",
  parameters = list(
    a1 = "log",
...
smart_runFun(
  ...
  args = list(
    ...
    sample_prior = "yes",
    init = 0,
    file = "./models/Model-Exp2/Models/Model_Exp2_M3_luce_free"
  ),
  untilFinished = FALSE,
  core = 4,
  maxCore = 4,
  name = "Model_Exp2_M3_luce_free"
)
