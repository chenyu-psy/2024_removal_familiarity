...
genM3Form <- function(choice_rule="softmax", model = "linear") {
  ...
  if (choice_rule=="softmax") {
    ...
    if (model == "linear") {
      M3_formula <- bf(
        ...
        nlf(TransOld ~ a + b),
        nlf(TransNew ~ a + b),
        nlf(OutSame ~ iP*(inv_logit(ra)*a + inv_logit(rc)*c1 + b) + (1-iP)*(-100)),
        nlf(OutOther ~ inv_logit(ra)*a + b),
        nlf(NPL ~ b),
...
genM3Prior <- function(choice_rule="softmax", model = "linear") {
  
  if (choice_rule=="softmax") {
    if (model == "linear") {
      M3_prior <- c(
        prior(normal(3,1), class = "b", nlpar = "a", lb = 0), 
...
smart_runFun(
  ...
  args = list(
    ...
    sample_prior = "yes",
    init = 0,
    file = paste0(model_path, "Model_Exp2_M3_Softmax_a")
    ),
  name = "Model_Exp2_M3_Softmax_a"
)
