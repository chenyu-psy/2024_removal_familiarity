...
genM3Form <- function(a1 = "free", c1 = "free", a2 = "free", 
  ...
  if (ra=="fix") {
    ...
    form_ra = ra ~ 0 + t + (0 + t || participant)
  }
  
  if (a1 == "fix"){
    form_a1 = a1 ~ 1 + (1 || participant)
  } else {
    form_a1 = a1 ~ 0 + t + (0 + t || participant)
  }
  ...
  if (a2=="fix") {
    ...
    form_a2 = a2 ~ 0 + t + (0 + t || participant)
  }
  ...
  final_form <- bf(
    ...
    nlf(mu6 ~ n + log(nNPL)),
    form_a1,
    c1 ~ 0 + t + (0 + t || participant),
    form_a2,
    c2 ~ 0 + t + (0 + t || participant),
    form_ra,
    rc ~ 0 + t + (0 + t || participant),
    n ~ 1,
    nl = TRUE)
...
luce_form <- bf(
  ...
  nlf(Correct ~ (1-iP)*(exp(a1) + exp(c1)) + iP*(exp(a2) + exp(c2)) + n),
  nlf(TransOld ~ exp(a1) + n),
  nlf(TransNew ~ exp(a2) + n),
  nlf(OutSame ~ iP*(inv_logit(ra)*exp(a1) + inv_logit(rc)*exp(c1)) + n),
  nlf(OutOther ~ inv_logit(ra)*exp(a1) + n),
  nlf(NPL ~ n),
  a1 ~ 0 + t + (0 + t || participant),
  c1 ~ 0 + t + (0 + t || participant),
  a2 ~ 0 + t + (0 + t || participant),
  c2 ~ 0 + t + (0 + t || participant),
  ra ~ 0 + t + (0 + t || participant),
  rc ~ 0 + t + (0 + t || participant),
  n ~ 1,
  nl = TRUE
)
...
luce_prior <- c(
  prior(normal(2,0.5), class = "b", nlpar = "a1"),
  prior(normal(2,0.5), class = "b", nlpar = "c1"),
  prior(normal(2,0.5), class = "b", nlpar = "a2"),
  prior(normal(2,0.5), class = "b", nlpar = "c2"),
  prior(normal(0,1), class = "b", nlpar = "ra"),
  prior(normal(0,1), class = "b", nlpar = "rc"),
  prior(constant(0.1), class = "b", nlpar = "n")
)
...
}, import = "auto", title = "Model_Exp2_M3_Luce")
...
final_form <- bf(
    ...
    nlf(mu2 ~ a1 + n + log(nTransOld)),
    nlf(mu3 ~ a2 + n + log(nTransNew)),
    nlf(mu4 ~ iP*(inv_logit(ra)*a1 + inv_logit(rc)*c1 + n) +log(nOutSame)),
    nlf(mu5 ~ inv_logit(ra)*a1 + n + log(nOutOther)),
    nlf(mu6 ~ n + log(nNPL)),
    nlf(a1 ~ ya1 + ba1*t),
    nlf(c1 ~ yc1 + bc1*t),
    nlf(a2 ~ ya2 + ba2*t),
    nlf(c2 ~ yc2 + bc2*t),
    nlf(ra ~ sra - (1-sra)*(1-exp(-exp(bra)*t))),
    nlf(rc ~ src - (1-src)*(1-exp(-exp(brc)*t))),
    form_a1,
    c1 ~ 0 + t + (0 + t || participant),
    form_a2,
    c2 ~ 0 + t + (0 + t || participant),
...
# The original model
Model_M3 <- read_rds(str_glue("{model_path}Model_Exp2_M3_Luce.rds"))

Post_M3 <- Model_M3 %>% 
  extractPost("t", par_name = "M3_par") %>% 
  filter(t!="Inercept") %>% 
  ...
  mutate(
    a1 = exp(a1),
    c1 = exp(c1),
    a2 = ifelse("a2" %in% names(.), exp(a2), a1),
    c2 = exp(c2),
