...
Table_Test_NPL <- Data_Test %>% 
  ...
  summarySE(DV = value, between = c("response","condition"), group = c("participant")) %>% 
  mutate( 
    # correct error bar
    upper = ifelse(abs(mean) < 1e-5, 0, mean + ci),
    lower = ifelse(abs(mean) < 1e-5, 0, mean - ci)) %>% 
...
(
  Plot_Test_NPL <- Table_Test_NPL %>% 
    ...
    ggplot(aes(x = condition, y = mean, color = response, fill=response))+
    geom_errorbar(
      aes(ymax = upper, ymin = lower),
      position = dodge2, 
      linewidth = 0.8,
