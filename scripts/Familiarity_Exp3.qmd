...
Table_Test_NPL <- Data_Test %>% 
  select(
    participant, condition, starts_with("NPL",ignore.case=FALSE)) %>%
  pivot_longer(
    cols = starts_with("NPL"), 
    names_to = "response", 
    values_to = "value"
    ) %>%
  summarySE(DV = value, between = c("response"), group = c("participant"))
...
(
  Plot_Test_Correct <- Table_Test %>% 
    ...
    labs(
      x = "", color = "Condition", fill = "Condition")+
    theme_test
)
...
(
  Plot_Test_NPL <- Table_Test %>% 
    ...
    ggplot(aes(x = response, y = mean, color = response, fill=response))+
    geom_errorbar(
      aes(ymax = upper, ymin = lower),
      position = dodge2, 
      linewidth = 0.8,
...
genM3Form <- function(phase="train") {
  ...
  if (phase=="train") {
    ...
    M3_formula <- bf(
        ...
        nlf(TransOld ~ a1 + iC*b2 + (1-iC)*b1),
        nlf(TransNew ~ a2 + iC*b1 + (1-iC)*b2),
        nlf(OutSame ~ iP*(inv_logit(ra)*a1 + inv_logit(rc)*c1 + iC*b2 + (1-iC)*b1) + (1-iP)*(-100)),
        nlf(OutOther ~ inv_logit(ra)*a1 + iC*b2 + (1-iC)*b1),
        nlf(NPLtrained ~ b2),
        nlf(NPLuntrained ~ b1),
        nlf(b2 ~ b + bb),
        nlf(b1 ~ b),
...
genM3Prior <- function(phase="train") {
    ...
    if (phase == "train") {
      ...
      M3_prior <- c(
        ...
        prior(normal(0, 1), class = "b", nlpar = "bb"),
        prior(constant(0), class = "b", nlpar = "b")
        )
    }
...
smart_runFun(
  ...
  args = list(
    ...
    sample_prior = "yes",
    init = 0,
    file = paste0(model_path, "Model_Exp3_M3_Testing")
    ),
  name = "Model_Exp3_M3_Testing"
)
